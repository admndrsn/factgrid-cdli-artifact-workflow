#!/usr/bin/env python3
"""Prepare vetted FactGrid missing period-claim additions from a live-state audit."""

from __future__ import annotations

import argparse
import re
from pathlib import Path

import pandas as pd


WORKFLOW_ROOT = Path(__file__).resolve().parents[1]
AUDIT_ROOT = WORKFLOW_ROOT / "published" / "period_audit"
DEFAULT_AUDIT = AUDIT_ROOT / "factgrid_period_claim_audit.csv"
DEFAULT_OUTPUT = AUDIT_ROOT / "factgrid_missing_period_claim_candidates.csv"
DEFAULT_SUMMARY = AUDIT_ROOT / "factgrid_missing_period_claim_candidates_summary.csv"


def clean(value: object) -> str:
    if value is None:
        return ""
    text = str(value).strip()
    return "" if text.lower() in {"nan", "none", "null"} else text


def read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        return pd.DataFrame()
    return pd.read_csv(path, dtype=str, low_memory=False).fillna("")


def cdli_key(value: object) -> str:
    digits = re.sub(r"\D+", "", clean(value))
    return digits.lstrip("0") or ("0" if digits else "")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--audit", type=Path, default=DEFAULT_AUDIT)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--summary", type=Path, default=DEFAULT_SUMMARY)
    args = parser.parse_args()

    audit = read_csv(args.audit)
    required = {
        "audit_status",
        "cdli_id",
        "factgrid_qid",
        "live_cdli_id",
        "expected_period_qid",
        "expected_period_label",
    }
    missing = required - set(audit.columns)
    if missing:
        raise RuntimeError(f"Audit CSV missing columns: {sorted(missing)}")

    rows = audit[
        audit["audit_status"].eq("missing_live_period")
        & audit["factgrid_qid"].map(clean).str.fullmatch(r"Q\d+", na=False)
        & audit["expected_period_qid"].map(clean).str.fullmatch(r"Q\d+", na=False)
    ].copy()
    rows["cdli_key"] = rows["cdli_id"].map(cdli_key)
    rows["live_cdli_key"] = rows["live_cdli_id"].map(cdli_key)
    rows = rows[rows["cdli_key"].eq(rows["live_cdli_key"]) & rows["cdli_key"].ne("")].copy()
    rows["period_property"] = "P853"
    rows["period_qid"] = rows["expected_period_qid"]
    rows["write_status"] = "ready"
    rows["write_note"] = "Add missing period claim expected from the current live FactGrid P692 CDLI ID."
    columns = [
        "write_status",
        "cdli_id",
        "live_cdli_id",
        "factgrid_qid",
        "artifact_label",
        "live_label_en",
        "live_description_en",
        "period_property",
        "period_qid",
        "expected_period_label",
        "write_note",
    ]
    rows = rows[[col for col in columns if col in rows.columns]].drop_duplicates()

    summary = (
        rows.groupby(["period_qid", "expected_period_label"], dropna=False)
        .size()
        .rename("count")
        .reset_index()
        .sort_values("count", ascending=False)
    )
    summary_total = pd.DataFrame(
        [
            {"metric": "missing_period_candidate_rows", "value": len(rows)},
            {"metric": "missing_period_candidate_items", "value": rows["factgrid_qid"].nunique() if not rows.empty else 0},
        ]
    )

    args.output.parent.mkdir(parents=True, exist_ok=True)
    rows.to_csv(args.output, index=False)
    pd.concat([summary_total, summary], ignore_index=True).to_csv(args.summary, index=False)
    print(f"missing period candidates: {len(rows)}")
    if len(rows):
        print(
            rows[
                [
                    "cdli_id",
                    "factgrid_qid",
                    "live_label_en",
                    "period_qid",
                    "expected_period_label",
                ]
            ]
            .head(30)
            .to_string(index=False)
        )
    print(f"Wrote candidates -> {args.output}")
    print(f"Wrote summary -> {args.summary}")


if __name__ == "__main__":
    main()
